...
public class ButtonHandler6 : MonoBehaviour {
    ...
    {
        if (currentDisplay.CurrentState == DisplayImage6.State.idle) return;

        if (currentDisplay.CurrentState == DisplayImage6.State.zoom)
        {
            currentDisplay.CurrentState = DisplayImage6.State.normal;
        ...
        else
        {
            currentDisplay.GetComponent<SpriteRenderer>().sprite
                = Resources.Load<Sprite>("Sprites/wall6" + currentDisplay.CurrentWall);
            currentDisplay.CurrentState = DisplayImage6.State.normal;
